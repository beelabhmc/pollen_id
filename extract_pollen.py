    ...
    for group in tqdm(
        ...
        for i, (index, row) in enumerate(group[1].iterrows()):
            ...
            if len(contours_filtered) == 0:
                logging.warning(
                    f"Skipping... no contours found on image: {row['path']}"
                )
                continue
        ...
        for i, c in enumerate(all_contours_combined):
            ...
            species_images.append(images_at_this_location)

    np.random.shuffle(species_images)
    for i in range(len(species_images)):
        prefix = ""
        if i < len(species_images) * 0.8:
            prefix = "train"
        else:
            prefix = "test"
